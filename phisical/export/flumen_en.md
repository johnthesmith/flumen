...
1.  10.5281/zenodo.14319493 <https://doi.org/10.5281/zenodo.14319493>
...
1.  Flumen (Latin flumen --- flow) is an abstract unidirectional
    connection between two natural numbers or indices (an oriented edge
    of a graph between two vertices).

2.  The flumen is denoted as  $f_{a,b}$, where $a \in \mathbb{N}$ is the
    first index, $b \in \mathbb{N}$ is the second index.

3.  Important: a flumen, like an edge of a graph, has no coordinates and
    does not belong to any space in the traditional sense, as
    $\mathbb{N} \subset \mathbb{R}, \mathbb{N} \neq 
                \mathbb{R}$, and also $x \in \mathbb{R}, x \notin 
                \mathbb{N}$, where $x$ is some spatial coordinate.

4.  A specific name was introduced to separate the perception of the
    flumen as a spatial quantum from the mathematical concept of a graph
    edge. At the same time, the flumen is the only \"building\" block
